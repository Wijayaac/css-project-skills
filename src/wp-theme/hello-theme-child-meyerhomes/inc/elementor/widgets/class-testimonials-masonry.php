...
class MH_Testimonials_Masonry_Widget extends Widget_Base
{
	...
	private function register_layout_controls(): void
	{
		...
		$this->add_responsive_control(
			...
			array(
				...
				'max'            => 5,
				'step'           => 0.1,
				'description'    => esc_html__('How many stacks fit in the viewport (e.g. 3.8 desktop, 2.5 tablet, 1.2 mobile).', 'hello-elementor-child'),
				)
		);
	...
	protected function render(): void
	{
		...
		$visible_desktop = isset($settings['visible_columns']) ? (float) $settings['visible_columns'] : 3.8;
		$visible_tablet  = isset($settings['visible_columns_tablet']) && $settings['visible_columns_tablet'] !== ''
			? (float) $settings['visible_columns_tablet']
			: 2.5;
		$visible_mobile  = isset($settings['visible_columns_mobile']) && $settings['visible_columns_mobile'] !== ''
			? (float) $settings['visible_columns_mobile']
			: 1.2;
		$visible_desktop = max(1.0, min(5.0, $visible_desktop));
		$visible_tablet  = max(1.0, min(5.0, $visible_tablet));
		$visible_mobile  = max(1.0, min(5.0, $visible_mobile));
...
?>
		<section
			class="mh-reviews"
			data-mh-reviews
			style="--mh-reviews-visible-desktop: <?php echo esc_attr((string) $visible_desktop); ?>; --mh-reviews-visible-tablet: <?php echo esc_attr((string) $visible_tablet); ?>; --mh-reviews-visible-mobile: <?php echo esc_attr((string) $visible_mobile); ?>;"
			<?php if (! $marquee) : ?>
			data-marquee="off"
			<?php endif; ?>>
