(function () {
  ...
  function getWpSessionToken() {
    ...
    localStorage.setItem("_wp_session", t);
    return t;
  }
  ...
  `;
    ...
    return gql(query, { cartId, lines }).then(function (res) {
  ...
  function removeLine(cartId, lineId) {
    ...
    return gql(query, { cartId: cartId, lineIds: [lineId] }).then(
      function (res) {
        var ue =
          (res.data &&
  ...
  function goCheckout(e) {
    ...
    setCartSession(cartId, token)
      ...
      .then(function (cart) {
        ...
        newTab.location.href = cart.checkoutUrl;
      })
      .catch(function (err) {
        console.error(err);
        alert(
    ...
    var html = cart.lines.edges
      .map(function (edge) {
        ...
        return (
          ...
          '<input type="number" class="qty" name="item-qty-[' +
          n.id +
          ']" value="' +
          n.quantity +
          '" min="1">' +
          ...
          "</div>" +
          '<button type="button" class="line-remove" aria-label="Remove"><span class="text-remove">Remove</span></button>' +
          "</div>" +
          "</div>"
        );
      })
  ...
  jQuery(function ($) {
    ...
    $(document)
      ...
      .on("submit.shopifyCart", "form.cart", function (e) {
        ...
        if (!gid) {
          alert(
            "This product is not mapped to Shopify and cannot be purchased.",
          );
          return false;
        }
        ...
        (existing ? Promise.resolve(existing) : ensureCart())
          ...
          .catch(function (err) {
            if (String(err).includes("does not exist")) {
              ...
              return ensureCart().then((newId) => addLines(newId, [line]));
            }
  ...
  function updateCartCount(cart) {
    ...
    var el = document.querySelector("#cart-toggle .number");
    if (el) {
      if (count > 0) {
  ...
  var id = getCartId();
  if (id) {
    getCart(id).then(updateCartCount);
  }
})();
